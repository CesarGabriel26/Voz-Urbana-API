...
def update_peticao(id):
    ...
    try:
        ...
        cursor.execute("""
            UPDATE peticoes 
            SET user_id = %s, causa = %s, content = %s, signatures = %s, required_signatures = %s, status = %s
            WHERE id = %s
        """, (updated_petition.user_id, updated_petition.causa, updated_petition.content, updated_petition.signatures, updated_petition.required_signatures, updated_petition.status, id))
